...
// 원하는 날짜 계산
  ...
  // 랭킹 정보를 저장할 state
  const [rankhobby, setrankhobby] = useState([]);  
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // API 호출
        const res = await axiosInstance.get(`api/v1/users/rank/4?year=${year}&month=${month}`);
        
        // API 호출 성공 시 데이터를 state에 저장
        setrankhobby(res.data.result.themeRankDataList);
