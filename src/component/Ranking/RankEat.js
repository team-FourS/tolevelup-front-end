...
// 원하는 날짜 계산
  ...
  // 랭킹 정보를 저장할 state
  const [rankEat, setrankEat] = useState([]);  
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // API 호출
        const res = await axiosInstance.get(`api/v1/users/rank/2?year=${year}&month=${month}`);
        
        // API 호출 성공 시 데이터를 state에 저장
        setrankEat(res.data.result.themeRankDataList);
