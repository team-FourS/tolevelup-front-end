...
// 원하는 날짜 계산
  ...
  // 랭킹 정보를 저장할 state
  const [rankCulture, setrankCulture] = useState([]);  
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // API 호출
        const res = await axiosInstance.get(`api/v1/users/rank/3?year=${year}&month=${month}`);
        
        // API 호출 성공 시 데이터를 state에 저장
        setrankCulture(res.data.result.themeRankDataList);
...
{Loading ? ( // 로딩 중인 경우 스피너를 렌더링
  ...
  <table className="rank_table">
    ...
    <div className="rankingbox">
              {rankCulture.map((rankItem, index3) => (
                <tr key={index3}>
